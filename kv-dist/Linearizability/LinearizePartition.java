            ...
            public void run() {
                ...
                while(true){
                    ...
                    if(!pq.isEmpty() && !pq.peek().sentAck){
                        System.out.println("Checking pq");
                        multicastBuf = ("ACK "+pq.peek().id).getBytes(StandardCharsets.UTF_8);
                        System.out.println(multicastReceiver.getInetAddress()+" "+multicastReceiver.getLocalPort());
            ...
            public void run() {
                while(true){
                    ...
                    try {
                        ...
                        String[] received = new String(packet.getData(), 0, packet.getLength()).split(" ");
                        System.out.println("server brd ack"+id + Arrays.toString(received)+packet.getPort()+multicastSender.getLocalPort());
                        lock.lock();
                        MapValue v = acks.get(received[1]);
                        v.acks+=1;
                        lock.unlock();
//                        acks.put(received[1],  + 1);
                    } catch (IOException e) {
                        e.printStackTrace();
                    }
                }
